<title>Cebu & Co. | Transaction history records</title>
<?php
      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Transaction history records</li>
          </ol>
        </div>
      </div>
    </div>
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
                <a class="btn btn-secondary float-sm-right btn-sm" href="transactions_print.php"><i class="fa-solid fa-print"></i> Print</a>
            </div>
            <div class="card-body">
              <table id="example1" class="table table-bordered table-hover text-sm">
                ...
                <tbody id="users_data">
                  <?php
                  $sql = mysqli_query($conn, "SELECT * FROM requisition r JOIN products p ON r.prod_ID=p.prod_ID JOIN users s ON r.emp_ID=s.user_Id JOIN suppliers sup ON p.supplier_ID=sup.supplier_ID WHERE r.status ORDER BY r.request_date");
                  ...
                  <!-- UPDATE MODAL -->
                  <div class="modal fade" id="restock<?= $row['prod_ID'] ?>">
                                  ...
                                  <div class="modal-body">
                                    <div class="form-group">
                                      ...
                                      <select name="prod_ID" id="" class="form-control" required>
                                        <option value="" selected disabled>Select product</option>
                                        <?php 
                                          $new_sql = mysqli_query($conn, "SELECT * FROM products WHERE prod_ID = ".$row['prod_ID']." ORDER BY prod_name ASC ");
                                          if(mysqli_num_rows($new_sql) > 0) {
                                            while ($row2 = mysqli_fetch_array($new_sql)) {
                                             echo '<option value="' . $row2['prod_ID'] . '" ' . ($row['prod_ID'] == $row2['prod_ID'] ? 'selected' : '') . '>' . $row2['prod_name'] . '</option>';
                                            }
                          ...
                          <div class="form-group">
                            ...
                            <select name="prod_ID" id="" class="form-control" required>
                              ...
                              <?php 
                                ...
                                FROM products 
                                WHERE prod_ID NOT IN (SELECT prod_ID FROM requisition WHERE emp_ID = $logged_in_user) 
                                ORDER BY prod_name ASC");
                                if(mysqli_num_rows($new_sql) > 0) {
                                  while ($row2 = mysqli_fetch_array($new_sql)) {
